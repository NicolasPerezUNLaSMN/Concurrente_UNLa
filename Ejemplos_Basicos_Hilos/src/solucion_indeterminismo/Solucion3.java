...
public class Solucion3 extends Thread{
	...
	public Solucion3(int inicio, int fin) {
	...
	public static void main(String[] args) {
		...
		for (int i = 0 ; i<numeroNucleos; i++) {
			...
			if ( i != numeroNucleos-1) { //Si no es el ultimo hilo
				hilos[i] = new Solucion3(arranca,termina); //Hija de Thread
				hilos[i].start();
				...
				hilos[i] = new Solucion3(arranca,ren); //Hija de Thread
				hilos[i].start();
